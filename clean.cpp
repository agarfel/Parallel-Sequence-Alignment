            ...
            for (int i = 0; i < num_rows; i++){ // COMPUTE ALL ROWS
                ...
                for(int j = 1; j<m; j++){
                    ...
                    if ((T1_prev[j-1].value >= T2_prev[j-1].value) && (T1_prev[j-1].value >= T3_prev[j-1].value)){
                        T1_curr[j].value = f(A[i], B[j]) + T1_prev[j-1];
                    ...
                    // T2
                    int t1 = T1_curr[j-1] - (g+h);
                    int t2 = T2_curr[j-1] - g;
                    int t3 = T3_curr[j-1] - (g+h);
                    if ((t1 >= t2) && (t1 >= t3)){
                        T2_curr[j].value = t1;
...
int run(std::vector<char> A, std::vector<char> B, int p){
    ...
    */
    ...
    for (int i = 0; i < p; i++){
        ...
        FinalAlignment += results[i].alignment;
    }
